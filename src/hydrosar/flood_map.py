...
def iterative(
    hand: np.array, extent: np.array, water_levels: np.array = np.arange(15), minimization_metric: str = 'ts'
):
...
def _get_cli(interface: Literal['hyp3', 'main']) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--estimator', type=optional_str, default='iterative', choices=available_estimators, help=estimator_help
    )
    parser.add_argument(
        '--water-level-sigma', type=float, default=3.0, help='Estimate max water height for each object.'
    )
    parser.add_argument(
