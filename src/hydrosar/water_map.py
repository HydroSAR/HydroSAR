...
def select_hand_tiles(
    tiles: Union[np.ndarray, np.ma.MaskedArray], hand_threshold: float, hand_fraction: float
) -> np.ndarray:
...
def _get_cli(interface: Literal['hyp3', 'main']) -> argparse.ArgumentParser:
    ...
    if interface == 'hyp3':
        ...
        parser.add_argument('--vv-raster', help='Sentinel-1 RTC GeoTIFF raster, in power scale, with VV polarization.')
    elif interface == 'main':
        parser.add_argument('out_raster', help='Water map GeoTIFF to create')
        # FIXME: Decibel RTCs would be real nice.
        parser.add_argument('vv_raster', help='Sentinel-1 RTC GeoTIFF raster, in power scale, with VV polarization')
        parser.add_argument('vh_raster', help='Sentinel-1 RTC GeoTIFF raster, in power scale, with VH polarization')
