...
def readInputs():
    ...
    parser.add_argument('-s', '--sigma', type=int, default=1.5,
...
def preprocess(path1, path2):
    ...
    if dataset1.GetDriver().GetDescription() != 'GTiff'\
            or dataset2.GetDriver().GetDescription() != 'GTiff':
    ...
    return ratio_image_fixed, georef, shape, ypad, xpad
...
def nonLocalMeans(unfilteredImg, sig):
    print('Performing Non Local Means denoising on the ratio image: ')
...
def write_geotiff(image, georef, outname, noData=None, dtype=gdal.GDT_Float32, colors=False, statistics=None):
    ...
    if colors and statistics is not None:
        ...
        if noData == None:
            noData = np.median(values)

        maxMedian = abs(np.max(medians))
        minMedian = abs(np.min(medians))
        positiveChange = cm.get_cmap('seismic', 2 * maxMedian + 10)
        negativeChange = cm.get_cmap('Blues', 2 * minMedian)

        median = statistics[noData - 1, 3]
        print('No Change Median: ', median)
        for cl in stats:
            cn = cl[0]
            val = cl[1]
            if val != median:
...
def main():
    ...
    if inputs.debug:
        os.mkdir(debug_path)
    (ratioImg, georef, ShapeOrig, ypad, xpad) = preprocess(
        inputs.PreImage, inputs.PostImage)

    write_image(remove_padding(ratioImg, ShapeOrig, ypad, xpad), os.path.join(
        inputs.result_path, 'ratio-image.png'))

    filtRatioImg = ratioImg
    if inputs.sigma > 0:
        filtRatioImg = nonLocalMeans(ratioImg, inputs.sigma)
    else:
        print('Given a sigma of 0, skipping filtering step...')
    ...
    # write_geotiff(wvFilt[0, 0], georef, os.path.join(
    #     inputs.result_path, 'swt-decomposition-level.tif'))
    ...
    write_geotiff(classed_image, georef, os.path.join(
        inputs.result_path, 'ChangeDetectionGeo.tif'), noData=int(noChange), dtype=gdal.GDT_Byte, colors=True, statistics=statistics)
